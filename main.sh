...
while true; do
  ...
  *)
    ...
    while true; do
      if sudo zerotier-cli listnetworks | grep $NETWORK_ID | grep OK; then
        break 2
      fi
      ...
      if [ $ITERATION -ge $MAX_ITERATIONS ]; then
        echo "Reached maximum number of iterations ($MAX_ITERATIONS). Exiting..."
        continue 2
      fi

      echo "Network not yet joined. Retrying in 0.5 seconds... (Iteration $ITERATION of $MAX_ITERATIONS)"
      sleep 0.5
    done
    ;;
  esac
